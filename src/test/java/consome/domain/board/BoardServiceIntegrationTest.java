...
public class BoardServiceIntegrationTest {
    ...
    void 게시판_생성시_DB에_저장된다() {
        ...
        //when
        Board board = boardService.create(categoryId, name, description, displayOrder);
    ...
    void 게시판_이름_변경시_DB에_저장된다() {
        ...
        Board renamedBoard = boardService.rename(board.getId(), newName);
    ...
    void 중복된_게시판_이름은_예외발생() {
        ...
        int displayOrder = 1;
        boardService.create(categoryId, name, description, displayOrder);

        //when & then
        assertThatThrownBy(() -> boardService.create(categoryId, name, "두 번째 게시판", 2))
    ...
    void 게시판_이름_수정시_중복되면_예외발생() {
        ...
        String description = "첫 번째 게시판";
        int displayOrder = 1;
        Board board1 = boardService.create(categoryId, name, description, displayOrder);

        String name2 = "게시판2";
        boardService.create(categoryId, name2, "두 번째 게시판", 2);

        //when & then
        assertThatThrownBy(() -> boardService.rename(board1.getId(), name2))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageContaining("이미 존재하는 게시판 이름입니다.");
    }
    ...
    void 게시판_이름_유효성_검사() {
        ...
        //when & then
        assertThatThrownBy(() -> boardService.create(categoryId, invalidName, description, displayOrder))
                .isInstanceOf(IllegalArgumentException.class)
